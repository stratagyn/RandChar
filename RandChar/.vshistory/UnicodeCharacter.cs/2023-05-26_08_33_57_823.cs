...
public static partial class UnicodeCharacter
{
    private static readonly ISet<UnicodeCategory> UnicodeCategories = new HashSet<UnicodeCategory>
    {
        UnicodeCategory.Control, UnicodeCategory.Format,
    ...
    public static int[] GetUIDs(UnicodeCategory category, params UnicodeCategory[] categories)
    {
        if (categories.Length == 0)
        {
            ...
            return UnicodeCharacters.Value[category];
        }

        var characters = UnicodeCharacters.Value;
        var combinedUIDs = new List<int>();

        if (characters.TryGetValue(category, out var UIDs))
            combinedUIDs.AddRange(UIDs);

        for(var i = 0; i < categories.Length; i++)
            if (characters.TryGetValue(categories[i], out UIDs))
                combinedUIDs.AddRange(UIDs);
